...
class DeepReinforcementLearning(BehaviouralModelInterface):
    ...
    def _train(self, terminal_state, step):
        ...
        current_queues_list = self._main_model.predict([current_states_0, current_states_1, current_states_2, current_states_3])
        ...
        future_queues_list = self._current_model.predict([new_current_state_0, new_current_state_1, new_current_state_2, new_current_state_3])
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            if not done:
                ...
                #new_queue = reward + DISCOUNT * max_future_queue
                new_queue = reward
            else:
                new_queue = -1  #reward

            current_queues = current_queues_list[index]
            current_queues = [ 0 if q < 0 else (q if q < 1 else 1) for q in current_queues]
        ...
        if terminal_state or step%10 == 1:
            self._current_model.set_weights(self._main_model.get_weights())
    ...
    def action(self, game_state):
        score = self._calculate_score(game_state[0], game_state[2], game_state[3])  #Reward
        model_state = self._game_to_model_state(game_state)
        if isinstance(self._previous_state, list):
            self._update_replay_memory((self._previous_state, self._choose_action_from_prediction(self._previous_prediction), score, model_state, game_state[0] == 0))    #Previous state, action taken, score it got, new state it let to, and if it is done (never)
            self._train(game_state[0] == 0, game_state[0])
...
#How do we get from store to network error? - you let each action predict the score it gonna get, you then change the score and fit
